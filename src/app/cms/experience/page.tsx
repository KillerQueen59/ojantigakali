...
export default function CmsExperiencePage() {
  const [items, setItems] = useState<Experience[]>(staticData.experiences)
  ...
  const addItem = () =>
    setItems((prev) => [...prev, { role: '', company: '', period: '', location: '', points: [''] }])
  ...
  return (
    <div style={{ maxWidth: 720, display: 'flex', flexDirection: 'column', gap: 28 }}>
      ...
      {items.map((exp, i) => (
        <Card key={i} onRemove={() => removeItem(i)} title={exp.role || `Role ${i + 1}`}>
          <Grid>
            <Field label="Role">
              <Input value={exp.role} onChange={(v) => update(i, 'role', v)} />
            </Field>
            <Field label="Company">
              <Input value={exp.company} onChange={(v) => update(i, 'company', v)} />
            </Field>
            <Field label="Period">
              <Input value={exp.period} onChange={(v) => update(i, 'period', v)} placeholder="e.g. Jan 2022 – Present" />
            </Field>
            <Field label="Location">
              <Input value={exp.location} onChange={(v) => update(i, 'location', v)} />
            </Field>
          </Grid>
