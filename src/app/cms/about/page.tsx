      ...
      <Section label="Facts">
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {data.facts.map((fact, i) => (
            <div key={i} style={{ display: 'grid', gridTemplateColumns: '48px 1fr 2fr', gap: 8 }}>
              <Input value={fact.emoji} onChange={(v) => setFact(i, 'emoji', v)} placeholder="emoji" />
              <Input value={fact.key}   onChange={(v) => setFact(i, 'key',   v)} placeholder="Label" />
              <Input value={fact.value} onChange={(v) => setFact(i, 'value', v)} placeholder="Value" />
            </div>
          ))}
        </div>
      </Section>
...
function Section({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      <label style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.07em', textTransform: 'uppercase', color: 'var(--window-text-muted)' }}>
        {label}
      </label>
      {children}
    </div>
